...
def dashboard_view(request):
    ...
    male_percentage = round((male_count / total_residents * 100), 1) if total_residents > 0 else 0
    female_percentage = round((female_count / total_residents * 100), 1) if total_residents > 0 else 0
    ...
    pwd_percentage = round((pwd_count / total_residents * 100), 1) if total_residents > 0 else 0
    voter_percentage = round((voter_count / total_residents * 100), 1) if total_residents > 0 else 0
    
    # ---------------- Total Certificate Claims ----------------
    total_certificates = CertificateLog.objects.count()
...
def get_officials_json(request):
    # Define default officials structure matching your model's POSITION_CHOICES and COMMITTEE_CHOICES
    default_officials = [
        {'position': 'Punong Barangay', 'committee': 'None', 'order': 1},
        {'position': 'Kagawad', 'committee': 'Appropriation', 'order': 2},
        {'position': 'Kagawad', 'committee': 'Peace & Order', 'order': 3},
        {'position': 'Kagawad', 'committee': 'Health & Sanitation', 'order': 4},
        {'position': 'Kagawad', 'committee': 'Youth & Education', 'order': 5},
        {'position': 'Kagawad', 'committee': 'Clean & Beautification', 'order': 6},
        {'position': 'Kagawad', 'committee': 'Women, Children, & Elderly', 'order': 7},
        {'position': 'Kagawad', 'committee': 'Ways & Means', 'order': 8},
        {'position': 'SK Chairman', 'committee': 'None', 'order': 9},
        {'position': 'Secretary', 'committee': 'None', 'order': 10},
        {'position': 'Treasurer', 'committee': 'None', 'order': 11},
    ]
    
    officials = BarangayOfficial.objects.all().order_by('order')
    
    # If no officials exist in database, create them with empty names
    if not officials.exists():
        for default in default_officials:
    ...
    data = [{
        ...
        'name': official.name or '',  # Ensure empty string if None
        ...
        try:
            ...
            for official_data in data:
                try:
                    ...
                    official.name = official_data.get('name', '').strip()  # Strip whitespace
                    official.save()
...
def resident_detail_modal(request, id):
    ...
    certificates_query = CertificateLog.objects.filter(resident=resident)
    
    if not certificates_query.exists():
        full_name = f"{resident.first_name} {resident.middle_name} {resident.last_name}"
        certificates_query = CertificateLog.objects.filter(resident_name__icontains=full_name)
    
    total_certs = certificates_query.count()
